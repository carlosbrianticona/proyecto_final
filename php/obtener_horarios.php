...
if (isset($_GET['id_dia']) && isset($_GET['deporte']) && isset($_GET['nrdecancha'])) { 
  ...
  $nrdecancha = intval($_GET['nrdecancha']);
    ...
    $sql = "SELECT hora_inicio, hora_finalizado FROM deporte_cancha_hora LEFT JOIN reserva ON deporte_cancha_hora.ID = reserva.id_deporte_cancha_hora 
            ...
            union 
            SELECT hora_inicio, hora_finalizado FROM deporte_cancha_hora LEFT JOIN reserva ON deporte_cancha_hora.ID = reserva.id_deporte_cancha_hora 
            LEFT JOIN deporte ON deporte.ID = deporte_cancha_hora.id_deporte  
            join cancelaciones on cancelaciones.id_reserva= reserva.ID 
            WHERE deporte_cancha_hora.id_dia = ? AND deporte.ID = ? AND deporte_cancha_hora.cancha = ? AND deporte_cancha_hora.activa_s_n = 'SI' 
            ORDER BY 1,2; ";
    
    $stmt = $conexion->prepare($sql);
    $stmt->bind_param('iiiiii', $id_dia, $deporte, $nrdecancha, $id_dia, $deporte, $nrdecancha); 
    $stmt->execute();
