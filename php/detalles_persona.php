...
<body>
    <div class="container mt-5">
        ...
        <form id="form-detalles" method="POST">
            ...
            <div class="mb-3">
                <label for="documento" class="form-label">Número de Documento (de 6 a 11 digitos)</label>
                <input type="text" class="form-control" id="documento" name="documento" pattern="[0-9]+" minlength="6" maxlength="11" value="<?php echo $persona['Numero_Documento']; ?>" required>
                <div class="error-message text" style="display:none; color:white;"></div>
            </div>
            <div class="mb-3">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (data.status === 'error') {
                            ...
                            $('#documento').next('.error-message').text(data.message).show();
                        } else {
                            $('#documento').css('border-color', '');
                            $('#documento').next('.error-message').hide();
                        }
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        console.error('Error al validar el documento:', textStatus, errorThrown);
                    }
                });
            } else {
                $('#documento').css('border-color', 'red');
                $('#documento').next('.error-message').text('Solo se permiten números.').show();
            }
        });
